...
export const AntiGravityCanvas: React.FC = () => {
  ...
  const initParticles = useCallback((width: number, height: number) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      newParticles.push({
        ...
        color: Math.random() > 0.9 ? '#ef4444' : '#ffffff', // Crimson red theme accent for Estratégia Chinesa
  ...
  const animate = useCallback((time: number) => {
    ...
    gradient.addColorStop(0, `rgba(239, 68, 68, ${pulseOpacity})`); // Red brand identity gradient glow
  ...
  return (
    <div 
      ref={containerRef} 
      className="absolute inset-0 -z-10 overflow-hidden bg-black cursor-crosshair"
